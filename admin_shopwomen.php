...
if(isset($_POST['add_womenproduct'])) {
	...
	$select_womenproduct_name = mysqli_query($conn, "SELECT name FROM shop_women WHERE name = '$name'") or die('Query failed!');
	
	if(mysqli_num_rows($select_womenproduct_name) > 0) {
		$message[] = 'Product Name is already added';
	}else{
		$add_womenproduct_query = mysqli_query($conn, "INSERT INTO `shop_women`(name, price, image) VALUES('$name', '$price', '$image')") or die('Query Failed!');
		
		if($add_womenproduct_query) {
...
if(isset($_GET['delete'])) {
	...
	$delete_image_query = mysqli_query($conn, "SELECT image FROM shop_women WHERE id = '$delete_id'") or die('Query failed!');
	$fetch_delete_image = mysqli_fetch_assoc($delete_image_query);
	unlink('uploaded/'.$fetch_delete_image['image']);
	mysqli_query($conn, "DELETE FROM shop_women WHERE id = '$delete_id'") or die('Query Failed!');
	header('location:admin_shopwomen.php');
}
...
if(isset($_POST['update_product'])) {
	...
	mysqli_query($conn, "UPDATE shop_women SET name = '$update_name', price = '$update_price' WHERE id = '$update_p_id'") or die('Query failed!');
	...
	if(!empty($update_image)) {
		if($update_image_size > 2000000) {
			...
			mysqli_query($conn, "UPDATE shop_women SET image = '$update_image' WHERE id = '$update_p_id'") or die('Query failed!');
...
<head>
<meta charset="utf-8">
<title>Admin-Products | Shop Women</title>
...
<section class="add-products">
   ...
   <form action="" method="post" enctype="multipart/form-data">
      ...
      <input type="submit" value="add product" name="add_womenproduct" class="btn">
   </form>
...
<section class="show-products">
	...
	?>
		<div class="box">
			...
			<div class="name"><?php echo $fetch_womenproducts['image']; ?></div>
			<div class="price">LKR <?php echo $fetch_womenproducts['price']; ?>/-</div>
			<a href="admin_shopwomen.php?update=<?php echo $fetch_womenproducts['id']; ?>" class="option-btn">Update</a>
			<a href="admin_shopwomen.php?delete=<?php echo $fetch_womenproducts['id']; ?>" class="delete-btn" onClick="return confirm('Are you certain you want to delete this product?');">Delete</a>
		</div>
	...
	<?php
		if(isset($_GET['update'])) {
			...
			$update_query = mysqli_query($conn, "SELECT * FROM `shop_women` WHERE id = '$update_id'") or die('Query failed!');
